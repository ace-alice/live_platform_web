...
import notLoginMessage from '@/messages/notLoginMessage'
...
service.interceptors.response.use(
  (response: any): any => {
    ...
    if (code < 200 || code > 300) {
      ...
      return Promise.reject('error')
    } else if (parseInt(response.data.code) === 0 || parseInt(response.data.code) === 14) {
      notLoginMessage(response.data.message || response.data.msg)
      return Promise.reject('error')
    } else if (parseInt(response.data.code) === 602 || parseInt(response.data.code) === 604) {
      notLoginMessage(response.data.message || response.data.msg)
      return response
    } else {
      return response
